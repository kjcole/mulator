...
for i in range(0, 5):
    loc["r{0}".format(i)] = (5 + i, 15)
for i in range(5, 9):
    loc["r{0}".format(i)] = (0 + i, 34)
...
def updatePanel(r, value):
    ...
    (x, y) = loc[r]
    print("\033[%d;%dH%06d" % (x+1, y+1, value), end='')
...
def pause(msg):
    ...
    if not confirm:
        return
    ans = input("\033[18;10H\033[0K{0}".format(msg))
    if ans[0:1] in ('a', 'A'):
        confirm = False
...
def cycle():
    ...
    addr = (iReg) % 1000
    if opcode == 0:
        return 0                     # stop instruction
    elif opcode == 1:
        reg[r] = mem[addr]           # load register
    elif opcode == 2:
        mem[addr] = reg[r]           # store register
    elif opcode == 3:
        reg[r] = addr                # load register immediate
    elif opcode == 4:
        reg[r] = mem[reg[addr]]      # load register indexed
    elif opcode == 5:
        reg[r] = reg[r] + reg[addr]  # add register
    elif opcode == 6:
        reg[r] = reg[r] - reg[addr]  # sub register
    elif opcode == 7:
        reg[r] = reg[r] * reg[addr]  # mul register
    elif opcode == 8:
        reg[r] = reg[r] / reg[addr]  # div register
    elif opcode == 10:
        pReg = addr
        updatePanel('pReg', pReg)    # jump unconditionally
    elif opcode == 11:
        if reg[r] == 0:
            pReg = addr
            updatePanel('pReg', pReg)       # jump if register zero
    updatePanel("r{0}".format(r), reg[r])   # the register affected
    return 1
